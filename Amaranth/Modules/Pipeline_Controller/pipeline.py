...
class Pipeline(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        with m.Elif((self.instr_flags_decode.isALUreg | self.instr_flags_decode.isALUimm) & (self.instr_flags_fetch.isALUreg | self.instr_flags_fetch.isALUimm)):
            
            with m.If((self.reg_addr_fetch.rs1_addr != 0) & (self.reg_addr_decode.rd_addr != 0) &
                    ((self.reg_addr_decode.rd_addr == self.reg_addr_fetch.rs1_addr) | 
                     (self.reg_addr_decode.rd_addr  == self.reg_addr_fetch.rs2_addr))):
            #acá el rd de lo que se está ejecut&o es igual al rs1 o rs2 de lo que se quiere ejecutar. Tengo que frenar hasta que se haya escrito
                m.d.comb += [self.fetch_enable.eq(0), self.decode_enable.eq(0), self.addr_builder_enable.eq(0), 
                             self.execute_enable.eq(1), self.retire_enable.eq(1)]
                
                m.d.comb += [self.fetch_mux.eq(0), self.decode_mux.eq(0), self.execute_mux.eq(0), 
